...
# (four between 2026-07-07 and 2026-07-18, cronicle-s3 on 2026-08-07,
# ha-polly on 2026-09-16) and
...
locals {
  homelab_backup_docker_prefixes = [
    ...
    # "cronicle" removed 2026-08-08 — its backup CronJob was retired outright
    # on 2026-08-07 (Cronicle's live data moved to its own bucket, see
    # cronicle-s3 below; recovery is bucket versioning, no snapshot writer
    # remains). Same ordering rule observed: writer deleted first. The final
    # NFS-era snapshot under cronicle/ stays readable to admins and expires
...
# ---------------------------------------------------------------------------
# ha-polly — Home Assistant's text-to-speech (Amazon Polly, voice Liam fr-CA)
# ---------------------------------------------------------------------------
#
# Added 2026-09-16. The voice assistant's TTS moved from Google Cloud (its
# fr-CA Chirp voices sound like France French) to Polly's Liam, the only
# male neural Québécois voice on the market that we already have an account
# for. Consumed by the legacy `tts: amazon_polly` YAML platform on ha-01
# (secrets.yaml); HA OS cannot federate, so this is a static key like the
# others above. SynthesizeSpeech is the only call the integration makes at
# runtime; DescribeVoices is what it uses at startup to validate the voice.
# No resource scoping is possible for either action.

resource "aws_iam_user" "ha_polly" {
  name = "ha-polly"
}

data "aws_iam_policy_document" "ha_polly" {
  statement {
    sid    = "PollySynthesize"
    effect = "Allow"
    actions = [
...
resource "aws_iam_user_policy" "ha_polly" {
  name   = "ha-polly-synthesize-only"
  user   = aws_iam_user.ha_polly.name
  policy = data.aws_iam_policy_document.ha_polly.json
}
