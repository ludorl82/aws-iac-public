...
# --- alerting --------------------------------------------------------------
#
# SNS email rather than the fleet's usual ntfy, for the same reason the deadman
# switch uses it: a delivery path that shares nothing with the homelab. A scan
# that failed to reach Drive should still be reportable when k3s is the thing
# that is broken.

resource "aws_sns_topic" "numeriseur" {
  name = "numeriseur-processor"
}

resource "aws_sns_topic_subscription" "numeriseur_email" {
  topic_arn = aws_sns_topic.numeriseur.arn
  protocol  = "email"
...
resource "aws_cloudwatch_metric_alarm" "numeriseur_dlq" {
  ...
  alarm_actions       = [aws_sns_topic.numeriseur.arn]
  ok_actions          = [aws_sns_topic.numeriseur.arn]
}
