...
# ---------------------------------------------------------------------------
# cronicle-data-example-com — Cronicle's live storage (S3 engine)
# ---------------------------------------------------------------------------
#
# Not a backup bucket: this IS Cronicle's primary datastore since 2026-08-07
# (pixl-server-storage S3 engine; the pod is stateless, the NFS PVC retired).
# A live KV store overwrites and deletes constantly, so like the deadman
# bucket it must NEVER get Object Lock — locked versions can't be reclaimed
# and the store would grow for the whole retention period.
#
# Versioning IS on, deliberately: with the encrypted backup CronJob retired
# (2026-08-07), noncurrent versions are the only recovery mechanism for a bad
# write or delete. The noncurrent rule below is what keeps that from becoming
# unbounded growth on a bucket this write-heavy.

resource "aws_s3_bucket" "cronicle_data" {
  bucket = "cronicle-data-example-com"
...
resource "aws_s3_bucket_versioning" "cronicle_data" {
  bucket = aws_s3_bucket.cronicle_data.id
...
resource "aws_s3_bucket_server_side_encryption_configuration" "cronicle_data" {
  bucket = aws_s3_bucket.cronicle_data.id
...
resource "aws_s3_bucket_public_access_block" "cronicle_data" {
  bucket = aws_s3_bucket.cronicle_data.id
...
resource "aws_s3_bucket_ownership_controls" "cronicle_data" {
  bucket = aws_s3_bucket.cronicle_data.id
...
# No `expiration` rule: current versions are the live data. Only noncurrent
# versions (the undo history) are reaped.
resource "aws_s3_bucket_lifecycle_configuration" "cronicle_data" {
  bucket = aws_s3_bucket.cronicle_data.id
